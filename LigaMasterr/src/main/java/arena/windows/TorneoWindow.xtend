...
class TorneoWindow extends SimpleWindow<TorneoModel> {
	...
	def void createTorneoPanel(Panel panel) {
		...
		new Button(panel) => [
			caption = "GrondoRanking"
			onClick[new RankWindow(this, modelObject.dtON).open]
			fontSize = 10
		]
		
		new Button(panel) => [
			caption = "Traspasos Hechos"
			onClick[new TransferWindow(this, modelObject.dtON).open]
			fontSize = 10
		]
