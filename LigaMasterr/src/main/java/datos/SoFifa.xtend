...
final class SoFifa {
	...
	def getJugadores(String string) {
		val document = Jsoup.connect("http://sofifa.com/players?keyword=" + string).userAgent("Mozilla").post
		...
		tabla.map [
			...
			jugador.nombre = data.get(1).select("a").get(0).text
	...
	def getEquipos(String string) {
		val document = Jsoup.connect("http://sofifa.com/teams?keyword=" + string +"&hl=es-ES").userAgent("Mozilla").post
